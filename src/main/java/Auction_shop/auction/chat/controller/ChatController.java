...
public class ChatController {
    ...
    public void processMessage(@DestinationVariable String roomId, Chat chat) {
        ...
        try {
            Long opponentId = chat.getYourId();
            ChatRoom findOpponent = chatRoomService.isOtherUserLeft(response.getUserId(), opponentId);

            //상대방이 채팅방을 나간 경우
            if (findOpponent == null) {
                SseEmitter emitter = sseConnection.getEmitter(opponentId);  // 상대방ID를 통해 상대방의 emitter를 가져옴
                if (emitter != null) {
                ...
                if (emitter == null) {  // 상대방이 접속 중이 아닌 상태일 때 메세지 큐에 저장 후 나중에 상대방이 접속 시 생성된 채팅방 번호 제공
                    ...
                    String idTypeChange = String.valueOf(opponentId);
                    messageQueue.opsForList().leftPush(idTypeChange, opponentId);  // 메세지 큐에 저장
                }
            }
